...
export function renderLegend(): HTMLElement {
  ...
  // Seasonal features (M41): a chart symbol, filled while an arrow works through it, only with the layer on.
  html += `<div class="legend-row features"><svg width="32" height="14"><circle cx="7" cy="7" r="6" fill="#fff" stroke="#5b6472" stroke-width="1.4" stroke-dasharray="2 2"/><text x="7" y="10.3" text-anchor="middle" font-size="9" font-weight="700" fill="#5b6472">L</text><circle cx="25" cy="7" r="6" fill="#334155" stroke="#fff" stroke-width="1.2"/><text x="25" y="10.3" text-anchor="middle" font-size="9" font-weight="700" fill="#fff">H</text></svg><span>Seasonal feature: hollow, then filled while an arrow works through it</span></div>`;
  html += `<p class="legend-note features">Drawn only with "Seasonal features" on: a fixture of the year's weather (an H or L as on a weather chart, a ring for the polar vortex) in the months it is present. Never a state and never a cause on the map: nothing is computed from it and no arrow starts or ends at it; the arrows the studies describe as a change in it fill it in.</p>`;
  ...
  // Hide unaffected regions (M42): the hollow circle is simply absent while the toggle is on.
  html += `<p class="legend-note hide">With "Hide unaffected regions" on, a place no connection has reached in the month shown is not drawn at all, not even as this hollow circle, and nor are the arrows on their way to it. It is left off because nothing on this map is acting on it in that month, never because nothing happens there; the drivers, the place you have clicked and the place a story is pointing at always stay.</p>`;
